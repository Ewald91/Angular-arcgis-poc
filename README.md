...
# Dockerstuff
build:
`docker build -t angularesrimap-image .`

run (with volume):
`docker run --name angularesrimap-container -ti -d -p 4200:4200 -v ${PWD}:/code angularesrimap-image` 

enter container:
`docker exec -ti angularesrimap-container /bin/bash`
